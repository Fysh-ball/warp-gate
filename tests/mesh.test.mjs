...
/**
 * app.js gates Create and Join behind a network-exposure modal. It is answered here,
 * not bypassed: pre-seeding the dismissal flag through storage would skip the thing
 * under test on the day somebody changes what it asks or what the button is called.
 *
 * The modal is raised asynchronously (create is clicked, join runs at boot), so this
 * polls rather than reading once. A tab that never raises it is a real failure, not a
 * tolerated one: every path through this suite creates or joins, and silently accepting
 * "absent" is how this helper would keep passing after the modal stopped appearing.
 */
async function dismissNetModal(tab, label) {
  await tab.waitFor("!!document.getElementById('net-continue') && !document.getElementById('net-modal').hidden",
    { timeout: 20000, label: `${label} network notice raised` });
  await tab.eval("document.getElementById('net-continue').click(); return true;");
  await tab.waitFor("document.getElementById('net-modal').hidden === true",
    { timeout: 10000, label: `${label} network notice dismissed` });
}
...
try {
  ...
  const a = await probedTab(browser, APP);
  ...
  await dismissNetModal(b, 'tab B');
  const c = await probedTab(browser, link);
  await dismissNetModal(c, 'tab C');
