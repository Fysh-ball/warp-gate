...
// EXTENSION COPY of public/js/signal.js. Every change is marked `EXTENSION PATCH` and is
// listed in extension/README.md. It is produced by extension/sync-from-public.mjs and
// checked by extension/drift-check.mjs, so diff this against public/js/signal.js and the only
...
export class Signal extends EventTarget {
  ...
  connect() {
    ...
    const url = api(`/api/events?room=${encodeURIComponent(this.roomId)}&token=${encodeURIComponent(this.token)}`);
  ...
  async bye() {
    ...
    try {
      await fetch(api('/api/bye'), { // EXTENSION PATCH: absolute URL
        method: 'POST',
        headers: { 'content-type': 'application/json' },
...
export async function fetchConfig() {
  const res = await fetch(api('/api/config'), { signal: AbortSignal.timeout(8000) }); // EXTENSION PATCH: absolute URL
