...
| `WG_AD_ORIGINS` | empty | Comma separated. Widens the **landing page's** CSP only. Read below. |
...
nothing else. Deliberately **not** `connect-src`: a sponsor that can open its own
connections is a sponsor that can report what it saw. It is matched against the
resolved filename, so no request path can carry it onto the gate, onto an asset, or
onto a legal page. Left empty, both documents get a byte-identical policy.
