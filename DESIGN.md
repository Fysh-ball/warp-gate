...
|---|---|---|
| Deploy host | A container on a small always-on Linux box | Reuses an existing `cloudflared` tunnel. A zero dependency app means there is no image build at all: a stock `node:22-alpine` with the source mounted read-only. |
...
~/projects/warp-gate/
  ...
  public/
    ...
    faq.html
    terms.html                 filled: Alberta, Canada; contact wg@fysh.site
    ...
    css/style.css
    js/app.js                  UI, onboarding, sever
    js/landing.js              the landing page
...
`wg@fysh.site`): those were the operator's decisions, not design decisions, and the
...
open was closed by the deployment itself (a container on a small Linux host, per the decisions
...
|---|---|---|
| Scope clarity | 18/20 | Greenfield, file layout fully determined. Deploy host still open at the time of scoring; now a container on a small Linux host. |
