...
function show(name) {
  ...
  if (receive && receiveNoteText) receive.hidden = !['onboarding', 'home'].includes(name);
...
function afterAgreement() {
  ...
  else if (stored && parseSecret(stored)) runFlow($('join-btn'), () => startJoin(stored));
  else show('home');
}
